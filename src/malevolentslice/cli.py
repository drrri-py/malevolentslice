...
def run_command(
    ...
    if not quiet:
        with Progress(
            ...
            task = progress.add_task(
                f"[bold cyan][0/{total_discovered}][/bold cyan] Initializing...",
                total=100,
            ...
            def progress_cb(
                ...
                progress.update(
                    ...
                    description=f"[bold cyan][{current_file_idx}/{total_files}][/bold cyan] {display_name}",
                    extra=ram_badge
                )
            ...
            ram_badge_final = (
                ...
                if summary.peak_memory_mb <= 100.0
                else f"[yellow]Peak RAM: {summary.peak_memory_mb:.1f} MB[/yellow] [yellow](ELEVATED)[/yellow]"
            )
            progress.update(
                task,
                completed=100,
                description=f"[bold green]✔ Processed {summary.total_files} file(s)[/bold green]",
    ...
    if not quiet:
        ...
        if summary.peak_memory_mb <= 100.0:
            ram_status = f"[bold green]{summary.peak_memory_mb:.2f} MB[/bold green] [green](STABLE)[/green] [dim]envelope <= 100 MB[/dim]"
        else:
            ram_status = f"[bold yellow]{summary.peak_memory_mb:.2f} MB[/bold yellow] [bold yellow](ELEVATED)[/bold yellow] [dim]> 100 MB threshold[/dim]"
